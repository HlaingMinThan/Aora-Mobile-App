...
const FormField = ({ title, value, onChangeText, otherStyles, keyboardType, placeHolder, onSubmitEditing, multiline, error, ...props }) => {
            ...
            <View className={`space-y-2 ${otherStyles}`}>
                ...
                <View className={`w-full h-16 border-2 border-black-200 bg-black-100 rounded-2xl focus:border-secondary ${multiline ? 'h-32' : 'h-16'}`}>
                    <TextInput
                        ...
                        className={`flex-1 text-white p-4  font-semibold text-base ${error ? ' border-red-500 border-[1px]' : ''}`}
                        value={value}
